...
router.get('/:reference', checkoutLimiter.middleware((req) => `${req.ip || 'unknown'}:${req.params.reference}`), asyncHandler(async (req, res) => {
  const link = await withTransaction(async (c) => {
    const found = (await c.query(
      `SELECT pl.id, pl.workspace_id, pl.amount, pl.checkout_fee, pl.currency, pl.status, pl.expires_at,
            w.merchant_id, w.provider_config_ref, w.webhook_endpoint_id, mo.return_url AS marketplace_return_url
       FROM payment_links pl
       JOIN workspaces w ON w.id = pl.workspace_id
       LEFT JOIN marketplace_orders mo ON mo.payment_link_id=pl.id
  ...
  const result = await provider.apm.startApm({
    ...
    notificationUrl,
    returnUrl: link.marketplace_return_url || `${config.appPublicBase}/payment-complete`,
    clientIp: req.ip,
  });
