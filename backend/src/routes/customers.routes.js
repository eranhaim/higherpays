...
router.get('/', requirePermission('customers.view'), asyncHandler(async (req, res) => {
  const { segment, q, creatorId } = req.query;
  ...
  if (creatorId) { vals.push(creatorId); where.push(`creator_id = $${vals.length}`); }
  ...
  const rows = await withWorkspace(wid(req), uid(req), async (c) => (await c.query(
    `SELECT id, alias, email, creator_id, segment, total_spend, last_purchase_at, created_at
...
router.get('/export', requirePermission('customers.export'), asyncHandler(async (req, res) => {
  const rows = await withWorkspace(wid(req), uid(req), async (c) => (await c.query(
    `SELECT c.alias, c.email, cr.stage_name AS creator, c.segment, c.total_spend, c.last_purchase_at
     FROM customers c LEFT JOIN creators cr ON cr.id = c.creator_id
  ...
  const csv = toCSV(
    ['alias', 'email', 'creator', 'segment', 'total_spend', 'last_purchase'],
    rows.map((r) => [r.alias, r.email, r.creator, r.segment, r.total_spend, r.last_purchase_at ? new Date(r.last_purchase_at).toISOString() : ''])
  );
...
router.get('/:id', requirePermission('customers.view'), asyncHandler(async (req, res) => {
  const row = await withWorkspace(wid(req), uid(req), async (c) => (await c.query(
    `SELECT id, alias, email, phone, country, creator_id, segment, tags, total_spend,
...
router.post('/', requirePermission('customers.manage'), asyncHandler(async (req, res) => {
  const { alias, email, phone, country, creatorId, segment, consentMarketing } = req.body || {};
  ...
  const created = await withWorkspace(wid(req), uid(req), async (c) => (await c.query(
    `INSERT INTO customers (workspace_id, creator_id, alias, email, phone, country, segment, consent_marketing, consent_recorded_at)
     VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7::customer_segment,'new'),COALESCE($8::boolean,false), CASE WHEN $8::boolean THEN now() END)
     RETURNING id, alias, email, creator_id, segment, created_at`,
    [wid(req), creatorId || null, alias, email || null, phone || null, country ? country.toUpperCase() : null, segment || null, consentMarketing || false])).rows[0]);
...
router.patch('/:id', requirePermission('customers.manage'), asyncHandler(async (req, res) => {
  const map = { alias: 'alias', email: 'email', phone: 'phone', country: 'country', segment: 'segment', creatorId: 'creator_id', consentMarketing: 'consent_marketing' };
  const sets = [], vals = [];
  ...
  const updated = await withWorkspace(wid(req), uid(req), async (c) => (await c.query(
    `UPDATE customers SET ${sets.join(', ')}
     ...
     RETURNING id, alias, email, creator_id, segment, updated_at`, vals)).rows[0]);
