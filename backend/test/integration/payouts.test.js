...
test('two concurrent payout runs record one payout, not two', async () => {
  ...
  await paySale(app, t, creator, 100);

  const run = () => request(app)
    .post(`/workspaces/${t.workspaceId}/payouts/run`)
    .set(t.authHeaders)
    .send({ payeeType: 'creator', targetId: creator.id });
  ...
  const payouts = await withSystem((c) => c.query(
    "SELECT status, amount FROM payouts WHERE workspace_id = $1 AND creator_id = $2", [t.workspaceId, creator.id]));
