...
export const authApi = {
  ...
  logout(refreshToken: string | null) {
    return api.post<void>('/auth/logout', refreshToken ? { refreshToken } : {}, { skipWorkspace: true });
  },

  /** Current user and every workspace they may sign into. */
  me() {
    return api.get<{ user: AuthUser; workspaces: AuthWorkspace[] }>('/auth/me', { skipWorkspace: true });
  },
